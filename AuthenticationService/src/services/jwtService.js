...
exports.generateNewAccessToken = (req,res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ userId: userId, userType: 'ADMIN' }, process.env.JWT_SECRET, { expiresIn: '1h' })
...
exports.getAccessToken = (userId,userRole, userName) => {

    const accessToken = jwt.sign({ userId: userId, userRole: userRole, userName:userName }, process.env.JWT_SECRET, { expiresIn: '3h' })

    return accessToken
}

//Generates refresh token
exports.getRefreshToken = (userId,userRole, userName) => {

    const refreshToken = jwt.sign({ userId: userId, userRole: userRole, userName:userName }, process.env.JWT_SECRET, { expiresIn: '90d' })
